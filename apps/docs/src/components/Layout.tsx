...
const searchFilter = (query: string, item: any) =>
  item.title.toLowerCase().includes(query.toLowerCase()) || item.body.raw.toLowerCase().includes(query.toLowerCase())

const NavLink = ({ children, href }: { children: string | ReactNode; href: string }) => {
  ...
  return (
    <a href={href} onClick={handleClick} className={`${styles.link} ${router.asPath === href && styles.activeLink}`}>
      {children}
    </a>
  )
}
...
export const Layout = ({ children }: { children: ReactNode }) => {
  ...
  const componentResults = allComponents.filter(component => searchFilter(query, component))
            ...
            <Stack space="12px" direction="column">
              ...
              <Stack space="12px" direction="column">
                {allComponents.map(component => (
                  <NavLink key={component.title} href={`/components/${component.slug}`}>
                    {component.title}
                  </NavLink>
                ))}
              </Stack>
